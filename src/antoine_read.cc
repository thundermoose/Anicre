...
#define FILENAME_TABLE "/tables.h"
	...
	ERROR("Memory allocation error.");
      ...
      strcpy(filename, _config._td_dir);
      strcat(filename, FILENAME_TABLE);

      if ((fid_table = fopen(filename, "w")) == NULL)
	{
	  perror("fopen");
	  ERROR("Failed to open '%s' for writing.", filename);
	}

      fprintf (fid_table, "/* This file is automatically generated. */\n");
      ...
      if (ferror(fid_table))
	{
	  ERROR("There has been an error during writing to '%s'.", filename);
	}

      if (fclose(fid_table) != 0)
	{
	  perror("fclose");
	  ERROR("Failed to close '%s' after writing.", filename);
	}
